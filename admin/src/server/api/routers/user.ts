...
const UserTypeEnum = z.enum(['EXTERNAL_USER', 'INTERNAL_EMPLOYEE']);
const ProductSeriesEnum = z.enum(['SEALOS', 'FASTGPT', 'LAF_SEALAF', 'AI_PROXY']);
export const userRouter = createTRPCRouter({
  ...
  createUser: privateProcedure
    .input(
      z.object({
        ...
        productSeries: z.array(ProductSeriesEnum).optional().default([])
      })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      await (globalPrisma as any).userAccountType.create({
        data: {
          userUid: user.uid,
  ...
  updateUserProductSeries: privateProcedure
    .input(
      z.object({
        id: z.string().min(1),
        productSeries: z.array(ProductSeriesEnum)
      })
    )
    .mutation(async ({ ctx, input }) => {
      const globalPrisma = ctx.db.globalPrisma;
      ...
      const existing = await (globalPrisma as any).userAccountType.findUnique({
      ...
      if (!existing) {
        ...
        await (globalPrisma as any).userAccountType.update({
          where: { userUid: user.uid },
          data: { productSeries: input.productSeries }
        });
      }
  ...
  getUserAccountType: privateProcedure
    .input(
      z.object({
        id: z.string().min(1)
      })
    ...
    .query(async ({ ctx, input }) => {
      ...
      const userAccountType = await (globalPrisma as any).userAccountType.findUnique({
        where: { userUid: user.uid }
      });
  ...
  recharge: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        BigInt(user.account.balance ?? 0) -
          BigInt(user.account.deduction_balance ?? 0) +
          BigInt(input.amount) * BigInt(1000000) <
        BigInt(0)
      ) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
...
export async function queryUserByWorkspaceId(
  ...
  const workspace = await (regionPrisma as any).workspace.findUnique({
    where: {
      id: workspaceId
    },
    include: {
  ...
  (workspace.userWorkspace as any[]).forEach((userWorkspace: any) => {
...
export async function queryUserByWorkspaceName(
  ...
  const workspaceList = await (regionPrisma as any).workspace.findMany({
    where: {
  ...
  (workspaceList as any[]).forEach((workspace: any) => {
    (workspace.userWorkspace as any[]).forEach((userWorkspace: any) => {
      userCrUidList.push(userWorkspace.userCrUid);
    });
  });
...
export async function queryUserDetailByUd(
  ...
  return {
    ...
    // 新增用户类型和产品序列信息
    userType: userAccountType?.userType ?? 'EXTERNAL_USER',
    productSeries: userAccountType?.productSeries ?? []
  };
}
export async function getLocalQuota(
  ...
  const userCr = await (regionPrisma as any).userCr.findUnique({
    where: {
      userUid: user.uid
    },
    include: {
  ...
  const workspaceList = (userCr.userWorkspace as any[]).map(
    (userWorkspace: any) => userWorkspace.workspace
  );
...
export function parseQuotaValue(input: string, type: 'cpu' | 'memory' | 'storage'): QuotaResult {
  let value: number;
