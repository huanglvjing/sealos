...
export default function GenerationPanel(props: TabPanelProps) {
  ...
  const formSchema = z.object({
    ...
    prefix: z.string(),
    rechargeType: z.enum(
      ['TEST_RECHARGE', 'COMPENSATION_RECHARGE', 'ACTIVITY_RECHARGE', 'CORPORATE_RECHARGE'],
      {
        errorMap: () => ({ message: '请选择兑换码类型' })
      }
    )
  });
  ...
  const formik = useFormik<TinputhSchema>({
    initialValues: {
      ...
      amount: 0,
      prefix: '',
      rechargeType: 'COMPENSATION_RECHARGE' as const
    },
    ...
    async onSubmit(values, actions) {
      ...
      if (!result.success) {
        ...
        if (fieldErrors.rechargeType) {
          message({
            title: fieldErrors.rechargeType[0],
            status: 'error'
          });
          return;
        }
      ...
      try {
        await generateMutation.mutateAsync({
          ...
          rechargeType: formik.values.rechargeType
        });
        onOpen();
      } catch (error) {
        message({
  ...
  const rechargeTypeOptions = Object.entries(RechargeCodeTypeLabels).map(([key, label]) => ({
    value: key,
    label
  }));
  ...
  const customSelectStyles = {
    control: (provided: any, state: any) => ({
      ...
      appearance: 'none',
      transitionProperty:
        'var(--chakra-transition-property-common, background-color, border-color, color, fill, stroke, opacity, box-shadow, transform)',
      transitionDuration: 'var(--chakra-transition-duration-normal, 0.2s)',
      border: '1px solid',
      borderColor: '#E8EBF0',
      background: '#F7F8FA',
      fontWeight: '400',
    ...
    option: (provided: any, state: any) => ({
      ...
      backgroundColor: state.isSelected
        ...
        ? '#F7FAFC' // 淡灰色背景
        : 'white',
      color: state.isSelected ? '#2D3748' : '#2D3748',
      '&:hover': {
        backgroundColor: '#F7FAFC' // 鼠标悬停时的淡灰色
      }
    }),
    menu: (provided: any) => ({
      ...provided,
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        >
          <Flex wrap={'wrap'} maxW={'550px'} gap={'16px'}>
            ...
            <FormControl display={'flex'} alignItems={'center'}>
              <FormLabel width={'90px'}>兑换码类型 *</FormLabel>
              <Box flex={'auto'}>
                <Select
                  styles={customSelectStyles}
                  options={rechargeTypeOptions}
                  placeholder="请选择兑换码类型"
                  value={rechargeTypeOptions.find(
                    (option) => option.value === formik.values.rechargeType
                  )}
                  onChange={(selectedOption) => {
                    const helper = fieldHelpersFactory('rechargeType');
                    helper.setValue(
                      (selectedOption?.value as
                        | 'TEST_RECHARGE'
                        | 'COMPENSATION_RECHARGE'
                        | 'ACTIVITY_RECHARGE'
                        | 'CORPORATE_RECHARGE') || 'COMPENSATION_RECHARGE'
                    );
                  }}
                  isSearchable={false}
                />
            ...
            <FormControl display={'flex'} alignItems={'center'}>
              ...
              <Input
                {...fieldInputPropsFactory('coment')}
                flex={'auto'}
                placeholder={t('redemptionCode:comment')}
              />
            </FormControl>

            <Flex w={'full'} justify={'flex-end'}>
              <Button type="submit" variant={'solid'} isLoading={generateMutation.isPending}>
                {t('common:generate')}
              </Button>
            </Flex>
          </Flex>
